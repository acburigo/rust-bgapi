...
impl MessageHeader {
    pub fn from_binary(cursor: &mut Cursor<&[u8]>) -> Result<MessageHeader, Error> {
        const HEADER_SIZE_BYTES: u8 = 4;

        if cursor.remaining() < HEADER_SIZE_BYTES as usize {
            return Err(Error::from(ErrorKind::UnexpectedEof));
        }

        let header = MessageHeader {
            message_type: cursor.get_u8(),
            payload_length: cursor.get_u8(),
            message_class: cursor.get_u8(),
            message_id: cursor.get_u8(),
        };
